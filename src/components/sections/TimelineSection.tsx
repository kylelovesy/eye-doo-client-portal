...
// Placeholder for your SvgIcon component and icon map
// const getTimelineEventIconSrc = (type: TimelineEventType) => `#${type}`;
const SvgIcon = ({ title }: { title: string }) => <div className="w-6 h-6 bg-gray-200 rounded-full flex items-center justify-center text-xs font-bold text-gray-600">{title.charAt(0)}</div>;
...
export const TimelineSection = ({ config, items, onAddEvent }: TimelineSectionProps) => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const newEvent: Omit<ClientTimelineEventFull, 'id'> = {
      ...
      startTime: Timestamp.fromDate(new Date(formData.get('eventTime') as string)),
          ...
          sortedEvents.map(event => (
            <div key={event.id} className="flex items-stretch mb-6">
              <div className="flex flex-col items-center mr-4">
                <div className="text-white font-bold w-20 text-center py-1 rounded-t-lg bg-[#4A90E2]">{event.startTime.toDate().toLocaleTimeString()}</div>
                <div className="w-px flex-grow bg-gray-300"></div>
              </div>
              <div className="bg-white rounded-lg shadow-md p-4 w-full">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <SvgIcon title={event.type} />
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
           ...
           <div className="mb-4">
            ...
            <input type="number" id="duration" name="duration" required placeholder="e.g., 60" className="mt-1 block w-full border-gray-300 rounded-md shadow-sm form-input" />
          </div>
          <div className="mb-4">
